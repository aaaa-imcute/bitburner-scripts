...
export async function main(ns) {
  let LocationName = ns.enums.LocationName;
  let LiteratureName = {};
  ...
  let FactionName = ns.enums.FactionName;
  ...
  function getRandomIntInclusive(min, max) {
    ...
    return (min + max) / 2;//expected value
  }
  ...
  let methods = ["nuke", "brutessh", "ftpcrack", "relaysmtp", "httpworm", "sqlinject"];
  let prices = [0, 7e5, 1.5e6, 5e6, 3e7, 2.5e8];
  for (let i = 0; i <= 5; i++) {
    let s = serverMetadata.filter(e => e.numOpenPortsRequired == i);
    let r = 0;
    for (let j of s) {
      r += Math.pow(2, toNumber(j.maxRamExponent));
    }
    ns.tprint(methods[i] + ".exe is worth " + ns.formatNumber(prices[i]));
    ns.tprint("it gives " + ns.formatNumber(r) + " gb of free ram on average");
    ns.tprint("which costs " + ns.formatNumber(r * 55000) + " without considering bn multipliers");
  }
}